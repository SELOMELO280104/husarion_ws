...
from .rl_landing_policy import QLandingPolicy, RelativeState, cascaded_velocity
...
class VisualTagFollower(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('minimum_detection_confidence', 0.35)
        ...
        # Pre-speed-estimator behavior: do not add target-velocity feed-forward.
        self.declare_parameter('velocity_feedforward_gain', 0.0)
        ...
        self.declare_parameter('enable_rl_landing', False)
        self.declare_parameter('enable_rl_search', False)
        self.declare_parameter('landing_start_depth', 3.0)
        self.declare_parameter('landing_touchdown_depth', 0.35)
        self.declare_parameter('landing_max_horizontal_speed', 2.0)
        self.declare_parameter('landing_descent_speed', 0.8)
        self.declare_parameter('landing_center_tolerance_pixels', 20.0)
        self.declare_parameter('landing_center_priority_depth', 3.0)
        self.declare_parameter('landing_horizontal_gain_multiplier', 4.0)
        self.declare_parameter('rl_policy_path', '')
        self.declare_parameter(
        ...
        self.enable_rl_landing = bool(
            self.get_parameter('enable_rl_landing').value)
        self.enable_rl_search = bool(
            self.get_parameter('enable_rl_search').value)
        self.landing_start_depth = float(
            self.get_parameter('landing_start_depth').value)
        self.landing_touchdown_depth = float(
            self.get_parameter('landing_touchdown_depth').value)
        self.landing_max_horizontal_speed = float(
            self.get_parameter('landing_max_horizontal_speed').value)
        self.landing_descent_speed = float(
            self.get_parameter('landing_descent_speed').value)
        self.landing_center_tolerance_pixels = float(
            self.get_parameter('landing_center_tolerance_pixels').value)
        self.landing_center_priority_depth = float(
            self.get_parameter('landing_center_priority_depth').value)
        self.landing_horizontal_gain_multiplier = float(
            self.get_parameter('landing_horizontal_gain_multiplier').value)
        policy_path = str(self.get_parameter('rl_policy_path').value)
        self.rl_policy = QLandingPolicy(policy_path or None)
        self.landing_state = 'DISABLED'
        self.landing_requested = False
        self.last_landing_velocity_xy = (0.0, 0.0)
        self.last_landing_seen = None
    ...
    def _landing_service(self, _, response):
        if not self.enable_rl_landing:
            response.success = False
            response.message = 'Landing is disabled for this launch.'
            return response
        self.landing_requested = True
        response.success = True
        response.message = 'Landing requested; descending when marker depth is valid.'
        return response
    ...
    def _publish_status(self, mode, ready, depth=None, error=None):
        ...
        payload = {
            ...
            'rl_landing_enabled': self.enable_rl_landing,
    ...
    def _tick_gazebo_velocity(self):
        ...
        now = time.monotonic()
        if self.enable_rl_search:
            observation = (self.latest_observation
                           if self._fresh_observation() else None)
            mode = 'RL_SEARCH_MARKER_VISIBLE' if observation is not None \
                else 'RL_SEARCH_EXTERNAL_CONTROL'
            self.current_mode = mode
            self._publish_status(mode, observation is not None,
        ...
        if observation is not None:
            ...
            if self.enable_rl_landing and self.landing_requested:
                velocity_x *= self.landing_horizontal_gain_multiplier
                velocity_y *= self.landing_horizontal_gain_multiplier
                horizontal_norm = math.hypot(velocity_x, velocity_y)
                if horizontal_norm > self.landing_max_horizontal_speed:
                    scale = self.landing_max_horizontal_speed / horizontal_norm
                    velocity_x *= scale
                    velocity_y *= scale
                self.last_landing_velocity_xy = (velocity_x, velocity_y)
                self.last_landing_seen = now
            if self.enable_rl_landing and self.landing_requested:
                relative = self.marker_relative_xy
                state = RelativeState(
                    float(relative[0]) if relative is not None else 0.0,
                    float(relative[1]) if relative is not None else 0.0,
                    tag_depth,
                    float(self.estimated_panther_velocity_xy[0]),
                    float(self.estimated_panther_velocity_xy[1]),
                    0.0,
                )
                action, trained = self.rl_policy.action(state)
                if trained:
                    velocity_x, velocity_y = action[0], action[1]
                    self.landing_state = 'RL_APPROACH'
                    mode = 'RL_LANDING_APPROACH'
                else:
                    velocity_x, velocity_y, landing_velocity_z = cascaded_velocity(
                        state, max_speed=self.max_horizontal_speed,
                        landing_height=self.landing_touchdown_depth)
                    self.landing_state = 'PI_FALLBACK'
                    mode = 'LANDING_PI_FALLBACK'
                if tag_depth <= self.landing_touchdown_depth:
                    velocity_x = velocity_y = 0.0
                    landing_velocity_z = 0.0
                    self.landing_state = 'TOUCHDOWN_HOLD'
                    mode = 'LANDING_TOUCHDOWN_HOLD'
        elif self._fresh_ground_depth():
            height_feedback = float(self.latest_ground_depth)
            if (self.enable_rl_landing and self.landing_requested
                    and self.last_landing_seen is not None
                    and now - self.last_landing_seen <= self.marker_timeout):
                velocity_x, velocity_y = self.last_landing_velocity_xy
                landing_velocity_z = 0.0
                mode = 'LANDING_MARKER_REACQUIRE'
            elif height_feedback >= self.visual_engage_altitude:
        ...
        velocity_z = 0.0 if landing_velocity_z is None else landing_velocity_z
        # Landing mode owns the vertical channel; never let the normal
        # follow-height hold overwrite a requested descent.
        if (self.enable_rl_landing and self.landing_requested
                and observation is not None
                and tag_depth > self.landing_touchdown_depth):
            centered = pixel_error is not None and max(
                abs(float(pixel_error[0])), abs(float(pixel_error[1]))) \
                <= self.landing_center_tolerance_pixels
            if tag_depth > self.landing_center_priority_depth:
                centered = True
            velocity_z = (-min(self.max_vertical_speed,
                               self.landing_descent_speed)
                          if centered else 0.0)
        elif (self.enable_rl_landing and self.landing_requested
              and observation is None):
            # Never climb or descend blindly after losing the marker.
            velocity_z = 0.0
        if height_feedback is not None:
            if not (self.enable_rl_landing and self.landing_requested):
