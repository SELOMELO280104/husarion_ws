...
class VisualTagFollower(Node):
    ...
    def __init__(self):
        ...
        # --- REPO POLICY HİBRİT İNİŞ DEĞİŞKENLERİ ---
    ...
    def _landing_service(self, _, response):
        ...
        if not self.landing_active:
            ...
            self.get_logger().info('İniş Yetkisi Alındı! Hibrit Kör İniş Başlatılıyor...')
        response.success = True
        response.message = 'Landing requested; executing hybrid blind descent.'
        return response
    ...
    def _tick_gazebo_velocity(self):
        ...
        if not self._search_enabled(now) and not self.landing_active:
            self.search_target_xy = None
        ...
        # =========================================================================
        # HİBRİT KÖR İNİŞ PROTOKOLÜ (Görsel Takip + Hız Vektörü ile Kör Dalış - ENU Z: Negatif = Aşağı)
        # =========================================================================
        velocity_z = 0.0
        if self.landing_active:
            # Dikey alçalma hızı (ENU sisteminde aşağı yön negatif)
            velocity_z = -0.8  

            if observation is not None:
                # 1. Aşama: Marker görünürken hassas görsel takip ve UGV hız feedforward
                (velocity_x, velocity_y), pixel_error = gazebo_visual_velocity(
                    observation['center'],
                    observation['shape'],
                    tag_depth,
                    self.focal_xy,
                    self.horizontal_gain * 1.5,
                    self.max_horizontal_speed,
                    target_velocity=self.estimated_panther_velocity_xy,
                    feedforward_gain=1.0,
                )
                self.last_landing_velocity_xy = (velocity_x, velocity_y)
                mode = 'LANDING_VISUAL_TRACKING'
            else:
                # 2. Aşama (Kör Dalış): Yakın mesafeden dolayı marker kayboldu, UGV hız vektörüne kilitlen
                velocity_x = float(self.estimated_panther_velocity_xy[0])
                velocity_y = float(self.estimated_panther_velocity_xy[1])
                ...
                mode = 'LANDING_BLIND_TOUCHDOWN'

            self.current_mode = mode
            self._publish_gazebo_velocity(velocity_x, velocity_y, velocity_z)
            ready = self._update_readiness(pixel_error if pixel_error is not None else None, tag_depth)
            self._publish_status(mode, ready, tag_depth, pixel_error if pixel_error is not None else None)
            return
        else:
    ...
    def _tick(self):
        ...
        # --- PX4 BACKEND İÇİN İNİŞ İVMESİ (NED: Aşağı Yön = Pozitif) ---
        if self.landing_active:
            acceleration_down = 0.5  # Pozitif aşağı yönlü alçalma ivmesi
            mode = 'BLIND_TOUCHDOWN_HOLD'
        else:
